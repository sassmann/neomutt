...
{
  if (ctx == Context)
    dot_object_header(fp, ctx, "Context", "#ffc0ff");
  else
    dot_object_header(fp, ctx, "Context", "#c040c0");
...
void dot_context_list(FILE *fp, struct ContextList *cl, struct ListHead *links)
{
  struct Context *prev = NULL;
  struct Context *np = NULL;
  STAILQ_FOREACH(np, cl, entries)
  {
    dot_context(fp, np, links);
    if (prev)
      dot_add_link(links, prev, np, "Context->next", false);
...
void dump_graphviz(const char *title)
{
  ...
  dot_context_list(fp, &contexts, &links);
